...
class CouchSqlDomainMigrator(object):
    ...
    def _map_form_ids(self, couch_form):
        ...
        case_blocks = extract_case_blocks(couch_form.form)
        for case in case_blocks:
            ...
            if '@userid' in case and case['@userid'] in self._id_map:
                case['@userid'] = self._id_map[case['@userid']]

            if 'create' in case:
                if 'owner_id' in case['create'] and case['create']['owner_id'] in self._id_map:
                    case['create']['owner_id'] = self._id_map[case['create']['owner_id']]
                # TODO: Add questions that store location IDs or user IDs
            ...
            if 'update' in case:
                ...
                #     { …
                #      "case": { …
                #               "update": {"case_name": "Andrea"}},
                #      "meta": { … },
                #      "name": "Andrea"}
                pass

        meta = couch_form.form['meta']
        if 'userID' in meta and meta['userID'] in self._id_map:
            meta['userID'] = self._id_map[meta['userID']]
